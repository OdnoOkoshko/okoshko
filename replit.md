...
### Фронтенд архитектура (минимальная)
- **Фреймворк**: React 18 с JSX (только основные зависимости)
- **Сборщик**: Vite для разработки и продакшена
- **Стили**: TailwindCSS (чистая настройка, без дополнительных библиотек)
- **Интеграция с данными**: @supabase/supabase-js для работы с Supabase
- **Структура**: Сверх-минимальная - только необходимые файлы (2025-01-21)
...
- **Фронтенд бэкенд**: Убран - только статический фронт (2025-01-21)

### Интеграционная архитектура
- **Единое окно**: Фронтенд объединяет заказы со всех площадок
- **Данные**: Получение через Supabase API напрямую
...
- Максимально облегчен проект: убраны все лишние зависимости и файлы (2025-01-21)
- Удален серверный код - только чистый фронтенд (2025-01-21)
